...
final class HomeViewController: BaseCollectionViewController<HomeViewModel> {
    ...
    init(viewModel: HomeViewModel, coordinator: HomeCoordinating?) {
    ...
    override func setupCollectionView() {
        ...
        dataSource = UICollectionViewDiffableDataSource(collectionView: collectionView) { [weak self] collectionView, indexPath, itemID in
            ...
            if itemID.hasPrefix("header") {
                ...
                let cell = collectionView.dequeue(HomeHeaderCell.self, for: indexPath)
            ...
            if itemID.hasPrefix("hero") {
                guard let content = self.currentContent else { return nil }
                let cell = collectionView.dequeue(HomeHeroCell.self, for: indexPath)
                cell.configure(with: content)
                return cell
            }
            ...
            if let index = itemID.dashboardIndex(prefix: "stat") {
                ...
                let cell = collectionView.dequeue(HomeStatCell.self, for: indexPath)
            ...
            if let index = itemID.dashboardIndex(prefix: "focus") {
                ...
                let cell = collectionView.dequeue(HomeFocusCell.self, for: indexPath)
                cell.configure(with: focusItem)
                return cell
            }
    ...
    private func didSelectNavigationItem(_ item: HomeDashboardNavItem) {
        ...
        case .more:
            debugTrigger.accept(())
        case .english:
            coordinator?.showEnglish()
        case .fit:
            coordinator?.showFit()
        case .todo:
            coordinator?.showTodo()
        default:
            break
        }
    }
